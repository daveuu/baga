...
class SAMs_old:
    ...
    def align(self, insert_size = False, 
        ...
        genome_fna = 'genome_sequences/%s.fna' % self.genome.sample_name

        records_to_write = []
        for seq_id,seq_array in self.genome.sequence.items():
            records_to_write += [_SeqRecord(_Seq(seq_array.tostring()), 
                    id = seq_id, description = self.genome.names[seq_id])]
    ...
    def IndelRealignGATK(self, 
        ...
        genome_fna = 'genome_sequences/{}.fna'.format(self.genome.sample_name)
...
class SAMs(_MetaSample):
    '''
    A collection of short read datasets that have been aligned to the same genome 
    sequence in Sequence Alignment/Map (SAM) format for variant calling using GATK.
    '''

    def __init__(self, sample_name, reads = False, genome = False, 
            inherit_from = False, **kwargs):
        module_name = __name__
        super(SAMs, self).__init__(sample_name, module_name, 
                inherit_from = inherit_from, **kwargs)
        
        if reads and genome and not inherit_from:
            # making a new one
            try:
                self.read_files = reads.trimmed_read_files
            except AttributeError:
                text = 'WARNING: baga was not used to quality-score trim these '\
                        'reads. Read trimming is recommended for most types of '\
                        'analysis. This can be achieved with the "trim()" method '\
                        'of the Reads class in the PrepareReads module.'
                print(text)
                try:
                    self.read_files = reads.adaptorcut_read_files
                except AttributeError:
                    text = 'WARNING: baga was not used to remove library preparation '\
                            'adaptor sequences from these reads. Adaptor removal is '\
                            'highly recommended so hopefully you already removed '\
                            'adaptor sequences! This can be achieved with the '\
                            '"cutAdaptors()" method of the Reads class in the '\
                            'PrepareReads module.'
                    self.read_files = reads.read_files
                    print(text)
                    print('continuing with these reads . . .')
                
                # currently baga CollectData includes path to reads in pairname keys to
                # read file pair values. Check and remove here
                for pairname, files in self.read_files.items():
                    if _os.path.sep in pairname:
                        self.read_files[pairname.split(_os.path.sep)[-1]] = files
                        del self.read_files[pairname]
    ...
    def align(self, insert_size = False, 
        ...
        genome_fna = 'genome_sequences/%s.fna' % self.genome_name
        ...
        e1 = 'Could not find "read_files" attribute. Before aligning to genome, '\
                'reads must be quality score trimmed. Please run trim() method '\
                'on this Reads instance.'
        ...
        for pairname,files in self.read_files.items():
            ...
            out_sam = _os.path.sep.join([local_alns_path_genome, 
                    '%s__%s.sam' % (pairname, self.genome_name)])
    ...
    def toBAMs(self, path_to_exe = False, force = False, max_cpus = -1):
        ...
        for pairname,SAM in self.aligned_read_files.items():
            BAM_out = SAM[:-4] + '.bam'
            if not _os.path.exists(BAM_out) or force:
                cmd = '{0} view -buh {1} | {0} sort -o {2}.bam -T {3}; '\
                        '{0} index {2}.bam'.format(path_to_exe, SAM, SAM[:-4], 
                        SAM[:-4].split(_os.path.sep)[-1])
                print('Called: %s' % cmd)
                processes.add( _subprocess.Popen(cmd, shell=True) )
                if len(processes) >= max_processes:
                    (pid, exit_status) = _os.wait()
    ...
    def sortIndexBAMs(self, path_to_exe = False, force = False, max_cpus = -1):
        ...
        for SAM in self.paths_to_BAMs_dd:
            BAM_out = SAM[:-4] + '_si.bam'
            if not _os.path.exists(BAM_out) or force:
                cmd = '{0} sort -o {2}_si.bam -T {3}_si {1}; {0} index {2}_si.bam'\
                        ''.format(path_to_exe, SAM, SAM[:-4], 
                        SAM[:-4].split(_os.path.sep)[-1])
                print('Called: %s' % cmd)
                processes.add( _subprocess.Popen(cmd, shell=True) )
                if len(processes) >= max_processes:
    ...
    def IndelRealignGATK(self, 
        ...
        genome_fna = 'genome_sequences/{}.fna'.format(self.genome_name)

        e1 = 'Could not find "paths_to_BAMs_dd_si" attribute. Before starting '\
                'GATK analysis, read alignments must have duplicates removed. '\
                'Please run: .toBAMS(), .removeDuplicates(), .sortIndexBAMs() '\
                'methods on this SAMs instance, or --deduplicate if using '\
                'baga_cli.py.'
