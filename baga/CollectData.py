...
class Genome(_MetaSample):
    ...
    def queryEntrezAssembly(self, search_term, user_email, 
        ...
        exclude_terms can be a list of terms included with NOT.
        ...
        if len(exclude_terms):
            search_term = '('+search_term+')'
            for term in exclude_terms:
                search_term += ' NOT ({})'.format(term)
        ...
        for summary in summaries:
            ...
            skip = False
            for term in exclude_terms:
                if term in summary['Organism']:
                    self.logger.log(PROGRESS, 'Exclude term "{}" found in "{}": '\
                            'skipping'.format(term, summary['Organism']))
                    skip = True
                    break
            if skip:
                continue
    ...
    def downloadNextAssembly(self, include_complete = True, 
            ...
            use_genome_identifier = False, save_to_disk = False, path = '.'):
        '''
        ...
        If no additional inspection is required, use save_to_disk and path 
        arguments to save immediately. On each call, the current genome 
        information is removed from the .info dict attribute. When all genomes 
        have been processed and the .info dict attribute is empty, 
        ...
        save_to_disk : bool
            call .saveLocal() after download and move to location specified 
            by path
        save_to_disk : str
            path to save genome to if save_to_disk is True
        force : bool
        ...
        use_genome_identifier : str, optional
            use this genome identifier to build filename instead of 
            assembly accession
        ...
        if refseq_only and info['collection'] == 'GenBank':
            ...
            use_filename = '{}.{}-{}.baga'.format(__name__, type(self).__name__, 
                    genome_identifier)
            use_path = _os.path.realpath(_os.path.sep.join([path,use_filename]))
            ...
            if _os.path.realpath(self.file_name) != use_path:
                try:
                    _os.makedirs(path)
                    self.logger.debug('Created folder: {}'.format(path))
                except OSError as e:
                    if e.errno == 13:
                        m = 'Could not create folder: {} (permission denied)'.format(path)
                        self.logger.error(m)
                        raise OSError(e)
                
                self.logger.log(PROGRESS, 'Moving to new path: {}'.format(use_path))
                _os.rename(_os.path.realpath(self.file_name), use_path)
            if retain_gbk:
                gbk_content.seek(0)
                gbk_filename = use_link.split('/')[-1].replace('.gz','')
                gbk_path = _os.path.sep.join([path,gbk_filename])
                self.logger.info('Also writing {}'.format(gbk_path))
                try:
                    self.logger.info('Writing genbank file to {}.'\
                            ''.format(gbk_path))
                    open(gbk_path, 'w').write(gbk_content.read())
                except OSError:
        ...
        with _tarfile.open(local_path, "r:gz") as tar:
            for member in tar:
                ...
                try:
                    ...
                    # or longer python array.array objects
                    contents = _array(typecode, contents.getvalue().decode('utf-8'))
    ...
    def _extractLoci(self, seq_record):
        ...
        ordinate_offset = 0
        GI_prophage = _re.compile('[Ii]sland|[Pp]hage|GI')
        for n,f in enumerate(seq_record.features):
            if f.type == 'CDS':
                ...
                # sometimes locus_tag is missing: fall-back to protein_id
                if 'locus_tag' in f.qualifiers:
                    use_locus_key = 'locus_tag'
                elif 'protein_id' in f.qualifiers:
                    use_locus_key = 'protein_id'
                    self.logger.warning('CDS without a locus_tag: falling back to '\
                            'protein_id')
                else:
                    self.logger.warning('A CDS feature (number {}) is missing '\
                            'both "locus_tag" and "protein_id". It will be '\
                            'ignored! (it does have: {})'.format(n+1, 
                            ', '.join(f.qualifiers)))
                    continue
    ...
    def getFromENA(self, run_acc_list, 
        ...
        for cnum,run_acc in enumerate(run_acc_list):
            ...
            while not success:
                ...
                result = _urlopen(query_url_base + rest_req).read()
